...
public class MemberController {
	...
	// 스피링 컨테이너가 만든 memverService를 주입시켜라
	@Autowired 
...
//	}
	...
	@PostMapping("/member/signUp_confirm")
	public String signupconfirm(MemberDTO mdto,Model model //모델에 (id,pw,email)담아서 데이터 전달
			) {
		
		
		//MemberService 비지니스 로직을 담당하는 클래스
		//new 키워드를 사용해 객체생성
		...
		model.addAttribute("now", time.format(date));
		model.addAttribute("new_member_id", mdto.getId());
		model.addAttribute("new_member_pw", mdto.getPw());
		model.addAttribute("new_member_email", mdto.getEmail());
